...
export async function getKinoboxPlayers(options: KinoboxOptions): Promise<KinoboxPlayer[]> {
  // const baseUrl = new URL(options.baseUrl || "https://kinobox.tv/")
  const baseUrl = new URL(options.baseUrl || "https://p.ddbb.lol/")
  ...
  const getIframeUrl = (iframeUrl: string, source: string): string => {
    ...
    if (config?.domain) {
      ...
      const domain = config.domain.startsWith("//") ? `https:${config.domain}` : config.domain
  ...
  for (const player of players) {
    if (!Object.keys(playersConfig).some((source) => source.toLowerCase() === player.source.toLowerCase())) {
